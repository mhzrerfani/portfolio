...
export default function prepLettersPlaylist(playList, playOptions) {
    const { charTable, state, options } = this, reverse = playOptions?.reverse ?? false, charTableCopy = reverse ? [...charTable] : [...charTable].reverse();
    ...
    const executor = async () => {
        ...
        const loop = async () => {
            ...
            if (!lastChar)
                ...
                lastResult = (await lastChar.type()) ?? false;
        };
