...
function getGoalStringText(from) {
    const { options, goalText } = this, goalArray = options.html ? htmlToArray(goalText) : textToLetterItems(goalText), diff = Math.max(0, from.length - goalArray.length);
...
function setChar(i, l, gl, ghosts) {
    const { charTable, options } = this;
    charTable.push(new Char(this, l ?? '', gl.value || options.space, ghosts, gl.type, i));
}
