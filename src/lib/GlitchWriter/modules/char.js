...
export default class Char {
    ...
    async type() {
        ...
        await promiseWhile(() => (!this.finished || options.endless) && !state.isPaused && !this.stop, loop);
        if (this.finished) {
            state.progress.increase();
    ...
    step() {
        ...
        if ((this.stepsLeft > 0 && this.l !== this.gl) ||
            (coinFlip(this.afterGlitchChance) && this.specialType !== 'whitespace') ||
    ...
    removeGhost() {
        const { writer, ghosts } = this, deleted = coinFlip() && ghosts[0].length > 0 ? deleteRandom(ghosts[0]) : deleteRandom(ghosts[1]);
        if (deleted)
            writer.state.nGhosts--;
    }
}
